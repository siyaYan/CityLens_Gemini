...
const App: React.FC = () => {
  ...
  const handleCapture = async (base64Image: string) => {
    ...
    try {
      ...
      // Even if detailed fetch fails, we might still show the name if Step 1 succeeded
      if (landmarkData) {
          setAppState(AppState.READY);
      } else {
    ...
    <div className="w-full h-screen bg-slate-900 text-white overflow-hidden flex flex-col">
      ...
      {(appState === AppState.ANALYZING || appState === AppState.ERROR) && capturedImage && (
        <div className="relative h-full w-full flex items-center justify-center bg-black">
           <img src={`data:image/jpeg;base64,${capturedImage}`} className="absolute inset-0 w-full h-full object-cover opacity-30" alt="Analyzing" />
           <div className="scan-line"></div>
           <div className="z-10 text-center space-y-4">
              <div className="inline-block animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-sky-500"></div>
              <h2 className="text-2xl font-display font-bold text-sky-400 tracking-widest animate-pulse">ANALYZING SCENE</h2>
              <p className="text-sky-200/70 font-mono text-sm">Identifying landmarks via satellite uplink...</p>
           </div>
        </div>
      )}

      {(appState === AppState.FETCHING_DETAILS || appState === AppState.READY) && capturedImage && landmarkData && (
        <LandmarkHUD 
            image={capturedImage} 
